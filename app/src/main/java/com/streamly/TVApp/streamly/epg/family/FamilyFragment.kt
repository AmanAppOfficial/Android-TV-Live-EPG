package com.streamly.TVApp.streamly.epg.family
...
import com.streamly.TVApp.streamly.ProgramGuideFragment
...
import com.streamly.TVApp.streamly.HomeBean
import com.streamly.TVApp.streamly.R
import com.streamly.TVApp.streamly.VerticalHomeBean
...
class FamilyFragment (context: Context): ProgramGuideFragment<FamilyFragment.SimpleProgram>() {
//class EpgFragment (context: Context): ProgramGuideFragment<EpgFragment.SimpleProgram>() {
    ...
    companion object {
        private val TAG = FamilyFragment::class.java.name
    }
    ...
    private fun getFirstApi(authToken: String) {
        ...
        apiInterface.getChannelGenre2("Bearer $authToken")
                ...
                .subscribe(
                        {
                    //success
                    it.data.family.forEach {
                        channels.add(SimpleChannel(it.id.toString(),SpannedString(it.callSign),it.logo,it.url))
                    }

                    Log.e("### familyList",it.data.family.size.toString())

                },{
                    //error
                    Log.e("responseCode" , it.toString());
...
//        return ProgramGuideSchedule.createScheduleWithProgram(id, startTime.toInstant(), endTime.toInstant(), true, scheduleName,
                SimpleProgram(id.toString(), description, metadata+"", playUrl))
    }
